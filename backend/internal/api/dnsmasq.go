...
func generateMainConfig() error {
	...
	// DHCP Range
	content.WriteString(fmt.Sprintf("dhcp-range=%s,%s,%s,%s\n",
		config.RangeStart, config.RangeEnd, config.SubnetMask, config.LeaseTime))
	
	// Gateway
	if config.Gateway != "" {
		content.WriteString(fmt.Sprintf("dhcp-option=option:router,%s\n", config.Gateway))
	}
	
	// DNS Server
	if config.DNSServer != "" {
		content.WriteString(fmt.Sprintf("dhcp-option=option:dns-server,%s\n", config.DNSServer))
	}
	...
	if config.EnablePXE {
		...
		if config.NextServer != "" {
			content.WriteString(fmt.Sprintf("dhcp-boot=tag:bios,%s,,%s\n\n", bootFile, config.NextServer))
		} else {
			content.WriteString(fmt.Sprintf("dhcp-boot=tag:bios,%s\n\n", bootFile))
		}
	...
	content.WriteString(fmt.Sprintf("dhcp-boot=tag:ipxe,http://%s/api/v1/pxe/init\n\n", serverIP))
	}
